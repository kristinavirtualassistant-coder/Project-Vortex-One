...
const SORTS = {
  address: 'p.address ASC', city: 'p.city ASC', year_built: 'p.year_built DESC NULLS LAST', units: 'p.units DESC NULLS LAST', sqft: 'p.sqft DESC NULLS LAST', created_at: 'p.created_at DESC'
};
...
router.get('/search', async (req,res) => {
  try {
    const page=intParam(req.query.page,1,1000000), pageSize=intParam(req.query.pageSize,25,100), offset=(page-1)*pageSize;
    const q=String(req.query.q||'').trim(), city=String(req.query.city||'').trim(), state=String(req.query.state||'').trim(), county=String(req.query.county||'').trim(), zip=String(req.query.zip||'').trim(), propertyType=String(req.query.propertyType||'').trim(), apn=String(req.query.apn||'').trim();
    const minUnits=req.query.minUnits!==undefined?Number(req.query.minUnits):null, maxUnits=req.query.maxUnits!==undefined?Number(req.query.maxUnits):null;
    const sort=SORTS[String(req.query.sort||'address')]||SORTS.address;
    const params=[req.user.orgId], where=['p.org_id=$1'];
