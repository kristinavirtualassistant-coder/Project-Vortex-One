...
const corsOptions={credentials:true,origin(origin,callback){if(!origin||allowedOrigins.has(origin))return callback(null,true);return callback(new Error('Origin not allowed'));}};
...
app.use('/api/foundation',foundationRoutes);app.use('/api/leads',leadsRoutes);app.use('/api/property',propertyRoutes);app.use('/api/owners',ownerRoutes);app.use('/api/contacts',contactsRoutes);app.use('/api/imports',importsRoutes);app.use('/api/imports/scan',importScanRoutes);app.use('/api/saved-searches',savedSearchRoutes);app.use('/api/duplicates',duplicatesRoutes);app.use('/api/tasks',tasksRoutes);app.use('/api/campaigns',campaignsRoutes);app.use('/api/dialer',dialerRoutes);app.use('/api/reports',reportsRoutes);app.use('/api/data-quality',dataQualityRoutes);
const io=new Server(server,{cors:corsOptions});io.use((socket,next)=>{try{const jwt=require('jsonwebtoken');const token=socket.handshake.auth?.token;if(!token||!process.env.JWT_SECRET||process.env.JWT_SECRET.length<32)throw new Error('Unauthorized');socket.user=jwt.verify(token,process.env.JWT_SECRET);next();}catch{next(new Error('Unauthorized'));}});io.on('connection',socket=>{socket.join('agent:'+socket.user.userId);socket.emit('ready',{userId:socket.user.userId});});
const dist=path.join(__dirname,'../../dist');app.use(express.static(dist,{index:'index.html',maxAge:process.env.NODE_ENV==='production'?'1d':0}));app.get('*',(req,res)=>{if(req.path.startsWith('/api/'))return res.status(404).json({error:{code:'NOT_FOUND',message:'Not found'}});res.sendFile(path.join(dist,'index.html'));});app.use((err,req,res,next)=>{console.error(err);if(res.headersSent)return next(err);res.status(500).json({error:{code:'INTERNAL_ERROR',message:'Internal server error'}});});
const port=Number(process.env.PORT||8080);server.listen(port,'0.0.0.0',()=>console.log('Vortex One listening on '+port));
