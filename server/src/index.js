...
app.all('/api/auth/*', async (req, res, next) => {
  try { const { toNodeHandler } = await import('better-auth/node'); const auth = await betterAuth.getBetterAuth(); return toNodeHandler(auth)(req, res, next); }
  catch (error) { return next(error); }
});

app.use(express.json({ limit: '25mb' }));
app.get('/api/health', async (req, res) => { try { await db.query('SELECT 1'); res.json({ ok: true, service: 'vortex-one' }); } catch { res.status(503).json({ ok: false, error: { code: 'DATABASE_UNAVAILABLE', message: 'Database unavailable' } }); } });
...
io.use((socket, next) => { try { const jwt = require('jsonwebtoken'); const token = socket.handshake.auth?.token; if (!token) throw new Error(); socket.user = jwt.verify(token, process.env.JWT_SECRET); next(); } catch { next(new Error('Unauthorized')); } });
