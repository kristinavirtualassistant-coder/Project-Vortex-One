...
(async()=>{
  const server=spawn(process.execPath,['server/src/index.js'],{env:{...process.env,PORT:'8080',JWT_SECRET:jwtSecret},stdio:['ignore','pipe','pipe']}); let stderr=''; server.stderr.on('data',c=>{stderr+=c.toString();});
  try{
    await waitForHealth();
    const register1=await request('/api/auth/register',{method:'POST',body:JSON.stringify({organization:`M1 Org ${suffix}`,name:'M1 Owner',email:email1,password})});
    assert.equal(register1.response.status,201,JSON.stringify(register1.body)); assert.equal(register1.body.user.role,'owner'); const token1=register1.body.token; const auth1={Authorization:`Bearer ${token1}`};
    ...
    const register2=await request('/api/auth/register',{method:'POST',body:JSON.stringify({organization:`M1 Org 2 ${suffix}`,name:'M1 Owner 2',email:email2,password})}); assert.equal(register2.response.status,201,JSON.stringify(register2.body)); const token2=register2.body.token;
    const dashboard2=await request('/api/foundation/dashboard',{headers:{Authorization:`Bearer ${token2}`}}); assert.equal(dashboard2.response.status,200,JSON.stringify(dashboard2.body)); assert.deepEqual(dashboard2.body.counts,{properties:0,owners:0,leads:0,calls:0,members:1});
    ...
    const crossOrgSearch=await request('/api/property/search?page=1&pageSize=10',{headers:{Authorization:`Bearer ${token2}`}}); assert.equal(crossOrgSearch.response.status,200,JSON.stringify(crossOrgSearch.body)); assert.equal(crossOrgSearch.body.total,0);
