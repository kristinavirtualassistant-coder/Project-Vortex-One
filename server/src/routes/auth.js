...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (!organization || !name || !email || !password || password.length < 8) return res.status(400).json({ error: { code:'VALIDATION_ERROR', message:'Organization, name, email and an 8+ character password are required' } });
    ...
    const org = await client.query('INSERT INTO organizations(name) VALUES($1) RETURNING id', [organization.trim()]);
    const hash = await bcrypt.hash(password, 12);
    const user = await client.query('INSERT INTO users(org_id,name,email,password_hash,role) VALUES($1,$2,$3,$4,$5) RETURNING id,org_id,name,email,role', [org.rows[0].id, name.trim(), normalized, hash, 'owner']);
    ...
    await client.query('INSERT INTO system_settings(organization_id) VALUES($1)', [org.rows[0].id]);
...
router.post('/login', async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT id,org_id,name,email,role,password_hash FROM users WHERE email=$1', [(email || '').toLowerCase().trim()]);
    if (!result.rows.length || !(await bcrypt.compare(password || '', result.rows[0].password_hash))) return res.status(401).json({ error: { code:'INVALID_CREDENTIALS', message:'Invalid email or password' } });
